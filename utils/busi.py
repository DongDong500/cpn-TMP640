...
class BUSI(data.Dataset):
    ...
    def get_anchor(self, target):
        ...
        tl = (h.min(), w.min())
        rb = (h.max(), w.max())
        pnt = np.array([ (tl[0] + rb[0] - 1024) / 2048 , (tl[1] + rb[1] - 1024) / 2048 ], dtype=np.float32)
    ...
    def _read(self, index):
        ...
        assert( img.size == target.size == (1024, 1024) )

        target = (target, self.get_anchor(target))

        return img, target

    def __init__(self, root_pth, datatype='BUSI_with_GT', lesions='benign', fold='v5/3',
        ...
        image_dir = os.path.join(self.root_pth, self.datatype, self.lesions)
        split_f = os.path.join(self.root_pth, self.datatype, 'splits', self.lesions,
        ...
        self.images = [os.path.join(image_dir, f"{self.lesions} ({x}).png") for x in file_names]
        self.masks = [os.path.join(image_dir, f"{self.lesions} ({x})_mask.png") for x in file_names]
...
if __name__ == "__main__":
    ...
    for ist in image_set_type:
        dst = BUSI(root_pth='/home/dongik/datasets', datatype='BUSI_with_GT', 
                        lesions='benign', fold='v5/3', image_set=ist, transform=transform, )
